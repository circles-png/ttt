...
struct Game {
    ...
    animation: [Board<DisplayPixel>; 12],
}
...
impl Game {
    fn new() -> Self {
        ...
        let current_player = if pins.a5.into_pull_up_input().is_high() {
            Player::Red
        } else {
            Player::Blue
        };
        Self {
            ...
            animation: {
                let mut iter = include_str!("animation")
                    .trim()
                    .split("\n\n")
                    .map(|display| {
                        let mut iter = display.lines().map(|line| {
                            let mut iter = line.chars().map(|char| match char {
                                'r' => DisplayPixel {
                                    red: true,
                                    blue: false,
                                },
                            ...
                            let result = from_fn(|_| iter.next().unwrap());
    ...
    fn run(mut self) {
        loop {
            self.display = self.animation[(millis() as usize / 100) % 12];
            // match self.state {
